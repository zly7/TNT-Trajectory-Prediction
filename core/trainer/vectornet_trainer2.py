...
class VectorNetTrainer(Trainer):
        ...
        for i, data in data_iter:
            ...
            if training:
                ...
                with torch.no_grad():
                    ...
                    loss = self.compute_loss(data)
                    metrics = self.eval_loss(data_t)
                    if not self.multi_gpu or (self.multi_gpu and self.cuda_id == 1):
                        self.write_log("Eval Loss", loss.item() / n_graph, i + epoch * len(dataloader))
                        self.write_log('test minADE', metrics['minADE'], i + epoch * len(dataloader))
                        self.write_log('test minFDE', metrics['minFDE'], i + epoch * len(dataloader))
                        self.write_log('test MissRate', metrics['MR'], i + epoch * len(dataloader))
    ...
    def eval_loss(self, data):
        out = self.model(data)
        y = data.y.view(-1, self.horizon * 2)  # batch : 60,算损失的函数肯定是平均了的
        forecasted_trajectories, gt_trajectories = {}, {}

        for i in range(y.size(0)):
            pred_y = out["pred"][i].view((-1, 2)).cumsum(axis=0).cpu().numpy()
            y_cur = y[i].view((-1, 2)).cumsum(axis=0).cpu().numpy()
            forecasted_trajectories[i] = [pred_y]
            gt_trajectories[i] = y_cur
        metric_results = get_displacement_errors_and_miss_rate(
            forecasted_trajectories, gt_trajectories, max_guesses = 1, horizon = 30, miss_threshold = 2.0
        )
